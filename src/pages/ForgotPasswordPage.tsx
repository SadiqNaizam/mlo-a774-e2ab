...
const ForgotPasswordPage: React.FC = () => {
  ...
  const onSubmit = async (data: ForgotPasswordFormValues) => {
    ...
    if (data.email === "test@example.com") { // Simulate known email
      ...
      form.reset();
      // Optional: navigate to login page or a confirmation page
      // navigate('/'); 
    } else if (data.email === "error@example.com") { // Simulate server error
        setFormError("An unexpected error occurred. Please try again later.");
    }
    else { // Simulate email not found
      // We typically don't reveal if an email exists for security reasons, 
      // so the success message is often generic. But for demo, we can show an "error"
      // For a real app, you'd likely show the same success message as above.
